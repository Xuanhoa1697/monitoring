...
class ReportAction(models.Model):
    ...
    def preview_ms(self, docids, data):
        ...
        return (
            report_model.with_context(active_model=self.model)
            .sudo(False)
            .preview_ms(docids, data)
        )

    @api.model
    def clean_report_attachments(self):
        report_attachments = self.env['ir.attachment'].search([('public', '=', True), ('res_model', '=', 'ir.actions.report')])
        report_attachments.unlink()
